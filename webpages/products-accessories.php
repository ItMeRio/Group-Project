...
if (isset($_GET['search'])) {
    ...
    $sql = "SELECT products_ID, product_name, price, img, color, brand, categories, section FROM products WHERE product_name LIKE '%$search%' AND categories = 'Accessories'";

} else {
    // Default query without search filter
    $sql = "SELECT products_ID, product_name, price, img, color, brand, categories, section FROM products where categories = 'Accessories'";
}
    ...
    function filterProducts() {
        ...
        $('.product').each(function () {
            ...
            if ((selectedBrands.length === 0 || selectedBrands.includes(brand)) &&
                ...
                category === 'Accessories') { // Check if the category is 'Offers'
                $(this).show();
